...
(() => {
        "use strict";
        // Oculta overlay de arranque (si el JS está cargando, esto debe desaparecer)
        try { const bf = document.getElementById("bootFail"); if (bf) bf.style.display = "none"; } catch {}
        ...
        // --- Reparación rápida: limpia datos del sitio (cuando un perfil queda con caché/estado malo) ---
        const clearSiteData = () => {
          const stores = [];
          ...
          // storage es el que eligió la app (local/session/mem). Lo incluimos por si es memoria.
          try { stores.push(storage); } catch {}
          for (const s of stores) {
            try { s.removeItem && s.removeItem(AUTH_KEY); } catch {}
            try { s.removeItem && s.removeItem(KEY); } catch {}
          }
        };
        window.__clearSiteData = clearSiteData;
        ...
        function setupAuth() {
          ...
          if (repairBtn) {
            repairBtn.onclick = () => {
              const ok = confirm(
                "Esto borrará los datos guardados de esta app SOLO en este navegador (cerdas, camadas, ventas, etc.). ¿Continuar?",
              );
              if (!ok) return;
              try { clearSiteData(); } catch {}
              location.reload();
            };
          }
...
// --- Dialog compat (para navegadores sin HTMLDialogElement / showModal) ---
const __dlgSupport__ = (el) => !!(el && typeof el.showModal === "function" && typeof el.close === "function");
let __dlgBackdropEl__ = null;
const __ensureDlgBackdrop__ = () => {
  if (__dlgBackdropEl__) return __dlgBackdropEl__;
  const b = document.createElement("div");
  b.className = "dlgBackdrop";
  b.addEventListener("click", () => {
    try { if (dlg2 && dlg2.hasAttribute("open")) dlg2.close(); } catch {}
    try { if (dlg && dlg.hasAttribute("open")) dlg.close(); } catch {}
  });
  document.body.appendChild(b);
  __dlgBackdropEl__ = b;
  return b;
};
const __showDialogFallback__ = (el) => {
  if (!el) return;
  const b = __ensureDlgBackdrop__();
  b.style.display = "block";
  el.classList.add("dlg-fallback-open");
  el.setAttribute("open", "");
  // Forzar display por si el CSS del navegador no lo aplica
...
const __closeDialogFallback__ = (el) => {
  if (!el) return;
  el.classList.remove("dlg-fallback-open");
  el.removeAttribute("open");
  el.style.display = "none";
  if (__dlgBackdropEl__) __dlgBackdropEl__.style.display = "none";
  if (el.__onEsc__) {
    document.removeEventListener("keydown", el.__onEsc__);
    el.__onEsc__ = null;
  }
};
const __patchDialog__ = (el) => {
  if (!el) return;
  if (!__dlgSupport__(el)) {
    // polyfill showModal/close
    el.showModal = () => __showDialogFallback__(el);
    el.close = () => __closeDialogFallback__(el);
  }
};
__patchDialog__(dlg);
__patchDialog__(dlg2);
        ...
        function openAlert({
          ...
          try {
            ...
            if (!dlg2 || !dlg2.hasAttribute || !dlg2.hasAttribute("open")) dlg2.showModal();
